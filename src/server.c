      ...
      // read and write
        ...
        while ((num_read = read(events[i].data.fd, buf, BUF_SIZE)) > 0) {
          ...
          if (msg_type == '0') {
            ...
            // uint16_t port_tesing = ntohs(port);
            // printf("port is %hu", port_tesing);

            // int ret_sprintf =
            //     sprintf(completed_msg, "%c%" PRIu32 "%" PRIu16 "%s",
            //     msg_type,
            //             ip, port, msg);
            // int ret_sprintf = snprintf(completed_msg, BUF_SIZE,
            // "%d%u%hu%s\n",
            ...
            memcpy(completed_msg, buf, 1);
            memcpy(completed_msg + 1, &ip, 4);
            memcpy(completed_msg + 5, &port, 2);
            memcpy(completed_msg + 7, buf + 1, strlen(buf) - 1);

            completed_msg[strlen(buf) + 6] = '\n';
            completed_msg[strlen(buf) + 7] = '\0';
            ...
            for (int i = 0; i < MAX_EVENTS; ++i) {
              ...
              if (connected_fds[i] != -1) {
                ...
                if (write(connected_fds[i], completed_msg, strlen(buf) + 7) !=
                    strlen(buf) + 7) {
                  handle_error("write");
                }
              }
